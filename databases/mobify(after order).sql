...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 19, 2023 at 08:55 AM
...
INSERT INTO `customerproduct` (`c_id`, `p_id`, `buy_qty`, `od_id`) VALUES
(5, 1, 5, 2);
...
ALTER TABLE `customerproduct`
  ADD PRIMARY KEY (`c_id`,`p_id`,`od_id`),
  ADD KEY `od_id` (`od_id`),
  ADD KEY `p_id` (`p_id`);
...
--
ALTER TABLE `customerproduct`
  ADD CONSTRAINT `customerproduct_ibfk_1` FOREIGN KEY (`od_id`) REFERENCES `orders` (`od_id`) ON DELETE CASCADE,
  ADD CONSTRAINT `customerproduct_ibfk_2` FOREIGN KEY (`c_id`) REFERENCES `customer` (`c_id`) ON DELETE CASCADE,
  ADD CONSTRAINT `customerproduct_ibfk_3` FOREIGN KEY (`p_id`) REFERENCES `product` (`p_id`) ON DELETE CASCADE;
